...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await axios
        .post(`${window.location.protocol}//${window.location.hostname}:8080/api/auth/register`, {
          username: username,
          password: password,
        }, {
          headers: {
    ...
    <div className="flex w-full h-screen justify-center items-center">
      <Card className="w-[300px]">
        <CardBody className="flex flex-col items-center pt-8">
          ...
          <form className="flex flex-col gap-4 w-11/12 mt-6 mb-3" onSubmit={handleRegister}>
            <Input
              required
              label="Username"
